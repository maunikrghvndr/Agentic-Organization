...
- **Build (first use in a repo, then incremental):** from the repo root, run `<invocation> extract . --code-only --update`. `--code-only` keeps extraction local and deterministic (tree-sitter AST, no API calls, no tokens); the first run builds the graph, later runs refresh only changed files. Add `--no-cluster` on rapid iteration and cluster periodically.
- **Where it lands:** Graphify has no output-path flag and always writes a `graphify-out/` folder (`graph.json`, `cache/`, ...) at the scanned root — i.e. the repo root. Do not try to relocate it (that breaks the `--update` cache). Add `graphify-out/` to the repo's `.gitignore`. It is a regenerable, derived artifact — never hand-edit it, and rebuild it if lost. Durable knowledge that cannot be regenerated from code still lives in `.ai-memory/` (`PROJECT_MEMORY.md`, `wiki/`, `sources/`).
...
- **Query before grepping:** with the resolved invocation, `<invocation> query "..."`, `<invocation> path A B`, `<invocation> explain X` to locate relevant files and concept→code paths.
