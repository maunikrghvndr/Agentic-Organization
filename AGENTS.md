...
- **Build:** with the resolved invocation, refresh the graph incrementally with `<invocation> extract . --code-only --update`. `--code-only` keeps extraction local and deterministic (tree-sitter AST, no API calls) — use it by default; add `--no-cluster` on rapid iteration and cluster periodically.
